...
app.get("/comics", async (req, res) => {
  try {
    ...
    if (search) {
      ...
      const response = await axios.get(
        `https://gateway.marvel.com/v1/public/comics?ts=${timestamp}&apikey=${public_API_key}&hash=${hash}&limit=100&offset=${offset}&orderBy=title`
      );
      res.json(response.data);
    }
  } catch (error) {
...
app.get("/characters", async (req, res) => {
  try {
    ...
    if (!search) {
      ...
      const response = await axios.get(
        `https://gateway.marvel.com/v1/public/characters?ts=${timestamp}&apikey=${public_API_key}&hash=${hash}&limit=100&offset=${offset}&orderBy=name&nameStartsWith=${search}`
      );
      res.json(response.data);
